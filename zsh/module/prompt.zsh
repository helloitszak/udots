...
prompt_pure_human_time_to_var() {
	...
	(( days > 0 )) && human+="${days} day "
	(( hours > 0 )) && human+="${hours} hr "
	(( minutes > 0 )) && human+="${minutes} min "
	human+="${seconds} sec"
...
prompt_pure_preprompt_render() {
    ...
    preprompt_parts+=('%F{${prompt_pure_colors[path]}}'"$dir"'%f')
	

	# Add Git branch and dirty status info.
	typeset -gA prompt_pure_vcs_info
	if [[ -n $prompt_pure_vcs_info[branch] ]]; then
		local branch="%F{$git_color}"'git:${prompt_pure_vcs_info[branch]}'
	...
	# Execution time.
	[[ -n $prompt_pure_cmd_exec_time ]] && preprompt_parts+=('%F{$prompt_pure_colors[execution_time]} ran for ${prompt_pure_cmd_exec_time}%f')
	...
	local -ah ps1
	ps1=(
		'%K{$prompt_pure_colors[background]} '
